...
function Lastconnexion {
    ...
    Write-Log
    Write-Output "derniere connexion de $lastuser" >> $info_log
    # Recherche la derniere connexion de l'utilisateur
    Invoke-Expression "$sshtarget lastlog -u $lastuser"
    Invoke-Expression "$sshtarget lastlog -u $lastuser" >> $info_log
}
...
function last_change_password() {
    $user = Read-Host "De quel utilisateur souhaitez vous voir la derniere modification du mot de passe ?"
    $message = "Date du dernier changement de mot de passe de l'utilisateur $user"
    Write-Log
    Invoke-Expression "$sshtarget 'chage -l $user | grep modification'" 
    Invoke-Expression "$sshtarget 'chage -l $user | grep modification'" >> $info_log
    $message = "fin d'affichage de la derniere fois que $user a changer de mot de passe"
    Write-Log $mesage
}
...
function sesionactive {
    ...
    Write-Log
    $session_name = Read-Host "Session active pour l'utilisateur"
    Write-Output "Liste des session actives pour $session_name" >> $info_log
    Invoke-Expression "$sshtarget 'grep session opened /var/log/auth.log | grep $session_name'"
    Invoke-Expression "$sshtarget 'grep session opened /var/log/auth.log | grep $session_name'" >> $info_log
}
...
function user_group() {
    ...
    $message = "affichage a quelle groupe appartient l'utilisateur $username_user"
    Write-Log $mesage 
    Write-Output "Appartenance des groupes pour $username_user" >> $info_log
    Invoke-Expression "$sshtarget 'cat /etc/group | grep $username_user'" 
    Invoke-Expression "$sshtarget 'cat /etc/group | grep $username_user'" >> $info_log
...
function last_cmd {
    ...
    $message = "affichage des commandes utilisé par $choix_users"
    Write-Log $mesage 
    Write-Output "Liste des commandes utilisé par $choix_users" >> $info_log
    Invoke-Expression "$sshtarget 'cat ~/.bash_history'" 
    Invoke-Expression "$sshtarget 'cat ~/.bash_history'" >> $info_log
...
function Droitsdossier {
    ...
    $droitdoss = Read-Host "Nom du dossier (dossier dans /home/user ou chemin complet)"
    $message = "Affichage des droits du dossier $droitdoss pour l'utilisateur $choix_users"
    Write-Log



    Write-Output "Droits de l'utilisateur $choix_users sur le dossier $droitdoss " >> $info_log
    
    Invoke-Expression "$sshtarget 'getfacl $droitdoss'" 
    Invoke-Expression "$sshtarget 'getfacl $droitdoss'" >> $info_log
}
...
function Filedroit {
    ...
    $droitfile = Read-Host "Nom du fichier (dossier dans /home/user ou chemin complet)"
    $message = "Affichage des droits du dossier $droitfile pour l'utilisateur $choix_users"
    Write-Log
    Write-Output "Droits de l'utilisateur $choix_users sur le dossier $droitfile " >> $info_log
    
    Invoke-Expression "$sshtarget 'getfacl $droitfile'" 
    Invoke-Expression "$sshtarget 'getfacl $droitfile'" >> $info_log
}
...
function add_user {
    ...
    if (Invoke-Expression "$sshtarget id wilder") {
...
$sshtarget = "ssh -t $choix_users@$choix_ordinateur"
