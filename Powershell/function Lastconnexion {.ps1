function Lastconnexion {
    ...
    $loginEvents = Get-WinEvent -FilterHashtable @{LogName='Security'; ID=4624} -MaxEvents 1000 Session $session 
    
    # Recherche la derniere connexion de l'utilisateur
    $lastLogin = $loginEvents | Where-Object { $_.Properties[5].Value -eq $lastuser } | Sort-Object TimeCreated -Descending | Select-Object -First 1 | Tee-Object $info_log -Append
