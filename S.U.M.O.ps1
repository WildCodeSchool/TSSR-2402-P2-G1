...
function Lastconnexion {
    ...
    $lastLogin = Invoke-Command -ScriptBlock { param ($lastuser) Get-WinEvent -FilterHashtable @{ LogName ='Security'; ID = 4624 } -MaxEvents 1000 |  Where-Object { $_.Properties[5].Value -eq "$lastuser" } | Sort-Object TimeCreated -Descending | Select-Object -First 1} -argumentlist $lastuser -Session $session
    $lastLogin = Invoke-Command -ScriptBlock { param ($lastuser) Get-WinEvent -FilterHashtable @{ LogName ='Security'; ID = 4624 } -MaxEvents 1000 |  Where-Object { $_.Properties[5].Value -eq "$lastuser" } | Sort-Object TimeCreated -Descending | Select-Object -First 1} -argumentlist $lastuser -Session $session >> $info_log
...
function last_change_password() {
    ...
    {   
        ...
        Invoke-Command -scriptBlock { param($user) (Get-LocalUser -Name $user).PasswordLastSet } -ArgumentList $user -Session $session 
        Invoke-Command -scriptBlock { param($user) (Get-LocalUser -Name $user).PasswordLastSet } -ArgumentList $user -Session $session >> $info_log
...
function sesionactive {
    ...
    Invoke-Command { query session } -Session $session 
    Invoke-Command { query session } -Session $session >> $info_log
...
function Droitsdossier {
    ...
    Invoke-Command -ScriptBlock { param ($droitdoss,$droituser) Get-Acl -Path $droitdoss | Select-Object -ExpandProperty Access | Where-Object { $_.IdentityReference -match "$droituser" } } -argumentlist $droitdoss,$droituser -Session $session 
    Invoke-Command -ScriptBlock { param ($droitdoss,$droituser) Get-Acl -Path $droitdoss | Select-Object -ExpandProperty Access | Where-Object { $_.IdentityReference -match "$droituser" } } -argumentlist $droitdoss,$droituser -Session $session >> $info_log
...
function Filedroit {
    ...
    Invoke-Command -ScriptBlock { param ($droitfile,$fileuser) Get-Acl -Path $droitfile | Select-Object -ExpandProperty Access | Where-Object { $_.IdentityReference -match "$fileuser" } } -argumentlist $droitfile,$fileuser -Session $session 
    Invoke-Command -ScriptBlock { param ($droitfile,$fileuser) Get-Acl -Path $droitfile | Select-Object -ExpandProperty Access | Where-Object { $_.IdentityReference -match "$fileuser" } } -argumentlist $droitfile,$fileuser -Session $session >> $info_log
...
function userEvtLog {
    ...
    Get-Content C:\Windows\System32\LogFiles\log_evt.log | Select-String $userevtlogg 
    Get-Content C:\Windows\System32\LogFiles\log_evt.log | Select-String $userevtlogg >> $info_log
...
function os_version()
{
    ...
    Invoke-Command -ScriptBlock { Get-WmiObject Win32_OperatingSystem | Format-Table Version } -Session $session >> $info_log
...
function nbDisk {
    ...
    Invoke-Command -ScriptBlock { Get-Disk | Select-Object disknumber } -Session $session 
    Invoke-Command -ScriptBlock { Get-Disk | Select-Object disknumber } -Session $session >> $info_log
...
function partGet {
    ...
    Get-Partition | Select-Object -Property PartitionNumber, DriveLetter, @{Name = "SizeGB"; Expression = { [math]::Round($_.Size / 1GB, 2) } }, FileSystem, DiskNumber -Session $session
    Get-Partition | Select-Object -Property PartitionNumber, DriveLetter, @{Name = "SizeGB"; Expression = { [math]::Round($_.Size / 1GB, 2) } }, FileSystem, DiskNumber -Session $session >> $info_log
...
function dossSize_use {
    ...
    $message = "Affichage de la taille du dossier $doss_name"
    Write-Log
    ...
    $doss_name = Read-Host "Entrez le chemin complet du dossier"
        ...
        # Convertir en Megabytes et arrondir à deux décimales
        $dossSizeMB = [math]::Round($dossSize / 1MB, 2)  
        # Afficher les résultat
        Write-Host "Espace utilisé: $dossSizeMB MB"
        Write-Host "Espace utilisé: $dossSizeMB MB" >> $info_log
        } 
        else {
...
function lecteur_list {
    ...
    Invoke-Command -ScriptBlock { [System.IO.DriveInfo]::GetDrives() | Select-Object Name,DriveType | Format-Table -AutoSize } -Session $session 
    Invoke-Command -ScriptBlock { [System.IO.DriveInfo]::GetDrives() | Select-Object Name,DriveType | Format-Table -AutoSize } -Session $session >> $info_log
...
function list_install()
{
    ...
    Invoke-Command -ScriptBlock { Get-Package | Select Name } -Session $session
    Invoke-Command -ScriptBlock { Get-Package | Select Name } -Session $session >> $info_log
...
function service_run()
{
    ...
    Invoke-Command -ScriptBlock { Get-Service | Where-Object {$_.Status -eq "running"} | Select-Object Name } -Session $session
    Invoke-Command -ScriptBlock { Get-Service | Where-Object {$_.Status -eq "running"} | Select-Object Name } -Session $session >> $info_log
...
function list_user_local()
{
    ...
    Invoke-Command -ScriptBlock { Get-LocalUser | Select-Object Name } -Session $session >> $info_log
...
function ram_total()
{
    ...
    Invoke-Command -ScriptBlock { Get-WmiObject Win32_PhysicalMemory | Format-Table capacity } -Session $session >> $info_log
...
function ram_use {
    ...
    Invoke-Command -ScriptBlock { Get-ComputerInfo | Select-Object @{Name = "RAM utilisé en GB"; Expression = { ("{0:F2}" -f ($_.OsFreePhysicalMemory / 1MB)) } } } -Session $session
    Invoke-Command -ScriptBlock { Get-ComputerInfo | Select-Object @{Name = "RAM utilisé en GB"; Expression = { ("{0:F2}" -f ($_.OsFreePhysicalMemory / 1MB)) } } } -Session $session >> $info_log
...
function compEvtLog {
    ...
    Write-Log
    Get-Content C:\Windows\System32\LogFiles\log_evt.log | Select-String $compevtlogg
    Get-Content C:\Windows\System32\LogFiles\log_evt.log | Select-String $compevtlogg >> $info_log
